...
public class FilesManager(Client client, DataDbContext context) {
    public Task<FileEntity?> TryGetFileAsync(string id) =>
        context.FindAsync<FileEntity>(id).AsTask();

    public Task<FileEntity?> TryGetFileAsync(string category, string service, string logs_name, string extension) =>
        TryGetFileAsync(FileEntity.GenerateId(category, service, logs_name, extension));
    
    /// <remarks>
    /// File path "<see cref="Constants.Directory_Data"/>/<paramref name="category"/>/<paramref name="service"/>/Name.<paramref name="extension"/>"
    /// </remarks>
    ...
    public async Task<FileEntity> SaveFileAsync(Action<Stream> saveTo, string category, string service, string logs_name, string extension) {
        ...
        // Save
        var info = new FileInfo(Path.Combine(Directory_Data, file.Path, $"{file.Name}.{file.Extension}"));
    ...
    public async Task<FileTelegramInfo> SendFileAsync(FileEntity entity, User user, string caption) {
        ...
        var input_file = await client.UploadFileAsync(Path.Combine(Directory_Data, entity.Path, $"{entity.Name}.{entity.Extension}"));
        var input = new InputMediaUploadedDocument(input_file,
