...
public class StartCommand(Client client, AppDbContext context) : ICommand
{
    ...
    public async Task Invoke(UpdateNewMessage update, TL.User user)
    {
        ...
        {
            await client.Messages_SendMessage(user, $"Hello {user.first_name}! Please send your invite code. (/invite code)", new Random().NextInt64());
            return;
        }
        
        await client.Messages_SendMessage(user, $"Hello {user.first_name}!", new Random().NextInt64(), reply_markup: App.MainReplyKeyboardMarkup);
    }
}